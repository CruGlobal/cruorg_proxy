...
func TestNormalizePath(t *testing.T) {
	for in, want := range map[string]string{
		...
		if got := proxy.NormalizePath(in); got != want {
			t.Errorf("proxy.NormalizePath(%q) = %q, want %q", in, got, want)
		}
	}
}
...
func TestMergeQuery(t *testing.T) {
	...
	for _, c := range cases {
		if got := proxy.MergeQuery(c.target, c.in); got != c.want {
			t.Errorf("proxy.MergeQuery(%q, %q) = %q, want %q", c.target, c.in, got, c.want)
		}
	}
}
